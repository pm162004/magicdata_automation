...
class TestSearchProject:
    ...
    def test_read_project(self):
        logger.info("Read project with a valid project name and checking success toaster")
        verify_valid_project_name().click()
        check_projects().click()

    def test_search_exact_match(self):
        logger.info("Running: test_search_exact_match")
        project_name = input_field.EXACT_PROJECT_NAME
        search_input_field().clear()
        search_input_field().send_keys(project_name)
        search_input_field().send_keys(Keys.ENTER)

        WebDriverWait(driver, 10).until(EC.presence_of_all_elements_located(
            (By.XPATH, "//div[@class='card card-border']//h3")
        ))
        visible_names = get_all_project_names()
        assert project_name != visible_names, f"{project_name} No projects found"

        logger.info("Exact match project found successfully.")

    def test_search_partial_match(self):
        logger.info("Running: test_search_partial_match")
        search_input_field().clear()
        search_input_field().send_keys(input_field.PARTIAL_PROJECT_NAME)
        search_input_field().send_keys(Keys.ENTER)

        visible_names = get_all_project_names()
        assert any(input_field.PARTIAL_PROJECT_NAME in name for name in visible_names), "Partial match failed"
        logger.info("Partial match project found successfully.")

    def test_search_case_insensitive(self):
        logger.info("Running: test_search_case_insensitive")
        search_input_field().clear()
        search_input_field().send_keys(selected_name.upper())
        search_input_field().send_keys(Keys.ENTER)

        visible_names = get_all_project_names()
        assert any(selected_name.lower() in name.lower() for name in visible_names), "Case insensitive search failed"
        logger.info("Case insensitive search worked as expected.")

    def test_search_leading_trailing_spaces(self):
        logger.info("Running: test_search_leading_trailing_spaces")
        search_input_field().clear()
        search_input_field().send_keys(" "+ input_field.EXACT_PROJECT_NAME +"   ")
        search_input_field().send_keys(Keys.ENTER)
        logger.info("Whitespace-tolerant search passed.")

    def test_search_special_characters(self):
        logger.info("Running: test_search_special_characters")
        refresh_page()
        search_input_field().clear()
        search_input_field().send_keys("!@#$%^&*")
        search_input_field().send_keys(Keys.ENTER)
        logger.info("Search with special characters correctly returned no results.")

    def test_search_deleted_project(self):
        logger.info("Running: test_search_deleted_project")
        refresh_page()
        search_input_field().clear()
        search_input_field().send_keys(selected_name)
        search_input_field().send_keys(Keys.ENTER)
        delete_btn().click()
        confirm_btn().click()
        assert check_success_msg_project_delete().text == validation_assert.SUCCESS_MSG_DELETE_PROJECT

        refresh_page()
        search_input_field().send_keys(selected_name)
        search_input_field().send_keys(Keys.ENTER)
        project_names = get_all_project_names()
        assert selected_name != project_names, "Deleted project should not appear in search"
        logger.info("Deleted project does not appear in search results.")

    def test_search_newly_created_project(self):
        logger.info("Running: test_search_newly_created_project")
        timestamped_name = f"NewProject {int(time.time())}"
        new_projects_btn().click()
        project_name_input_field().send_keys(timestamped_name)
        description_input_field().send_keys(input_field.VALID_DESCRIPTION)
        save_btn().click()
        assert check_success_msg_project_create().text == validation_assert.SUCCESS_MSG_CREATE_PROJECT

        refresh_page()
        search_input_field().send_keys(timestamped_name)
        search_input_field().send_keys(Keys.ENTER)
        visible_names = get_all_project_names()
        assert "NewProject" != visible_names, "Newly created project not found in search"
        logger.info("New project appears in search results after creation.")

    def test_valid_edit_project_search(self):
        logger.info("Running: Edit existing project and verify search with updated name")
        check_projects().click()
        time.sleep(2)
        project_names = get_all_project_names()
        assert project_names, "No project names found to edit."

        global selected_name
        selected_name = random.choice(project_names)
        partial_name = selected_name.split()[0]
        logger.info(f"Selected project to edit: {selected_name}")
        search_input_field().clear()
        search_input_field().send_keys(partial_name)
        search_input_field().send_keys(Keys.ENTER)
        time.sleep(1)
        ...
        updated_name = f"{selected_name} Edited"
        project_name_input_field().clear()
        project_name_input_field().send_keys(updated_name)
        update_btn().click()
        ...
        check_projects().click()
        search_input_field().clear()
        search_input_field().send_keys(updated_name)
        search_input_field().send_keys(Keys.ENTER)
        time.sleep(2)

        visible_projects = get_all_project_names()
        assert updated_name in visible_projects, f"Updated project name '{updated_name}' not found in search results"
        logger.info(f"Updated project '{updated_name}' found successfully in search")

    def test_search_with_partial_name(self):
        logger.info("Running: Search project using partial name")

        check_projects().click()
        time.sleep(2)

        project_names = get_all_project_names()
        assert project_names, "No project names available."

        selected_name = random.choice(project_names)
        partial_name = selected_name[:len(selected_name)//2]
        logger.info(f"Searching using partial name: {partial_name}")
        ...
        results = get_all_project_names()
        assert any(partial_name in name for name in results), f"No match found for partial name '{partial_name}'"
        logger.info("Partial name search returned expected results.")
        quit_browser()
...
class TestDeleteProject:
    ...
    def test_valid_project_toaster_search(self):
        ...
        project_name_input_field().send_keys(selected_name)
    ...
    def test_valid_delete_project_search(self):
        logger.info("Running test: Valid project name flow")
        assert verify_valid_project_name().text == selected_name
        f"Project name mismatch: expected '{selected_name}', got '{verify_valid_project_name().text}'"
        logger.info(f" Project '{selected_name}' appears correctly in the UI")


    def test_cancel_project_delete_confirmation(self):
        logger.info("Running test: Cancel delete confirmation")

        delete_btn().click()
        logger.info("Delete icon clicked, confirmation dialog should appear")
        cancel_btn().click()
        logger.info("Cancel clicked in delete confirmation dialog")
        project_names = get_all_project_names()
        assert selected_name in project_names, f"Project '{selected_name}' should still exist after canceling delete"
    def test_multiple_deletes_sequentially(self):
        logger.info("Running test: Multiple project deletions in a row")

        check_projects().click()

        project_names = get_all_project_names()
        assert len(project_names) >= 2, "Need at least two projects to run this test"

        to_delete = project_names[:2]
        logger.info(f"Attempting to delete projects: {to_delete}")

        for project in to_delete:
            search_input_field().clear()
            search_input_field().send_keys(random.choice(project))
        ...
        all_remaining = get_all_project_names()
        for project in to_delete:
            assert project not in all_remaining, f"{project} was not successfully deleted"

    def test_delete_from_multiple_tabs(self):
        logger.info("Running test: Delete project from multiple tabs simultaneously")
        ...
        signin_btn().click()
        check_projects().click()
        search_input_field().send_keys(random.choice(selected_name))
        search_input_field().send_keys(Keys.ENTER)

        delete_btn().click()
        confirm_btn().click()
        logger.info("Deleted project from second tab")
        ...
        project_names = get_all_project_names()
        assert selected_name not in project_names, "Project should be gone in first tab too"
        logger.info("Project deletion reflected across tabs correctly")

    def test_delete_already_deleted_project(self):
        logger.info("Running test: Attempt to delete already deleted project")
        delete_btn().click()
        confirm_btn().click()
        assert check_success_msg_project_delete().text == validation_assert.SUCCESS_MSG_DELETE_PROJECT
        ...
        search_input_field().send_keys(Keys.ENTER)
        project_names = get_all_project_names()
        assert selected_name not in project_names, f"Project '{selected_name}' should not exist after deletion"
        logger.info("Verified deleted project does not appear in UI on second attempt")
        quit_browser()

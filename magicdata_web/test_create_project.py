...
class TestCreateProject:
    ...
    def test_multilanguage_description(self):
        logger.info("Running test: non-English input in description")
        refresh_page()
        project_name_input_field().send_keys(randomeString.random_username)
        description_input_field().send_keys("यह एक प्रोजेक्ट विवरण है")
        save_btn().click()
        assert check_success_msg_project_create().text == validation_assert.SUCCESS_MSG_CREATE_PROJECT
        new_projects_btn().click()

        logger.info("Multi-language input handled correctly")
